...
test('slow nonlocal composite displays only the component through inflection and keeps the admissible arc separate', async () => {
  ...
  assert.match(section, /buildFirstIntersectionCompositeFromRarefactionArc\(\s*compositeSourceRarefactionSegments,/)
  assert.match(section, /anchorCompositeContinuationAtInflection\([\s\S]*?rarefactionEnd,/)
  assert.match(section, /if \(rarefactionEnd && !hasUsableSegments\(globalNonlocalCompositeSegments\)\)[\s\S]*?extractOrientedCompositeArcFromFullCurve/)
  assert.doesNotMatch(section, /buildGlobalCompositeIntersectionSegments\(\s*anchor,/)
  assert.doesNotMatch(section, /buildCompositeFromSpecifiedRarefactionCurve\(/)
