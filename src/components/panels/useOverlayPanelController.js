...
import { readCaseDrawings, saveCaseDrawings } from './schaefferCaseStorage.js'
...
export default function useOverlayPanelController(props) {
  ...
  const [schaefferShearerCasePresets, setSchaefferShearerCasePresets] = useState(() => {
    const saved = readCaseDrawings()
    return Object.fromEntries(Object.entries(initialSchaefferShearerCasePresets)
      .map(([key, preset]) => [key, { ...preset, ...saved[key] }]))
  })
  useEffect(() => {
    saveCaseDrawings({
      ...schaefferShearerCasePresets,
      [selectedSchaefferShearerCase]: {
        scales: { yScale, tauScale, zScale },
        resolution,
        view,
      },
    })
  }, [schaefferShearerCasePresets, selectedSchaefferShearerCase, yScale, tauScale, zScale, resolution, view])
  ...
  const applySchaefferShearerCase = (caseKey) => {
    ...
    if (!preset || caseKey === selectedSchaefferShearerCase) return
    ...
    setTauScale(drawingDefault.scales.tauScale)
    setZScale(drawingDefault.scales.zScale)
    setResolution(drawingDefault.resolution)
    setView({ ...drawingDefault.view })
    setSchaefferShearerCasePresets((prev) => ({
      ...prev,
      [selectedSchaefferShearerCase]: {
        ...prev[selectedSchaefferShearerCase],
        scales: { yScale, tauScale, zScale },
        resolution,
        view: { ...view },
      },
  ...
  const resetCaseScales = () => {
    const defaults = drawingDefaultForCase(selectedSchaefferShearerCase)
    setYScale(defaults.scales.yScale)
    setTauScale(defaults.scales.tauScale)
    setZScale(defaults.scales.zScale)
    updateSelectedSchaefferShearerPreset((preset) => ({
      ...preset,
  ...
  const updateResolution = (value) => {
    setResolution(value)
    updateSelectedSchaefferShearerPreset((preset) => ({ ...preset, resolution: value }))
  }

  const resetCaseResolution = () => {
    updateResolution(drawingDefaultForCase(selectedSchaefferShearerCase).resolution)
  }
